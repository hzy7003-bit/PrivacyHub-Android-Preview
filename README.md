...
现阶段，下载并实际体验隐私中转站后，可申请对应设备的免费永久 Pro：

1. 在 App 的 Pro 页面复制本机设备码。
2. 将设备码和一段真实使用反馈或 ROM 兼容性报告发送至 [hzy7003@gmail.com](mailto:hzy7003@gmail.com?subject=%E9%9A%90%E7%A7%81%E4%B8%AD%E8%BD%AC%E7%AB%99%20Pro%20%E7%94%B3%E8%AF%B7)。
3. 审核后，开发者会通过邮件发送该设备对应的离线激活码。

> **请勿在 Issue、Discussion 或截图中公开设备码。Star 完全自愿，不是申请、审核或激活 Pro 的条件。**
...
- **低功耗常驻设计**：空闲时不轮询、不持有唤醒锁、不访问网络，也不默认监听剪贴板；通知栏入口按用户选择通过系统前台服务维持。
...
隐私中转站把后台职责限制为维持用户主动开启的通知栏快捷入口。空闲期间不执行定时扫描、网络请求、剪贴板轮询或重复数据库写入，也不持有唤醒锁；只有用户点击“保存到安全箱”或“保存并跳转”时，才短暂完成读取、解析、加密保存和路由。
